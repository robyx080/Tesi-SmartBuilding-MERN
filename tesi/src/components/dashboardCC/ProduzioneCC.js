...
function ProduzioneCC() {
    ...
    useEffect(()=>{
        ...
        const ConPro = async () =>{
            ...
            const response1 = await axios.post('http://localhost:5000/statProCC', {email,ora});
            setData(ora)
        ...
        const chart = async () => {
            ...
            let arrProdSol = [];
            let arrProdEol = [];
            const response = await axios.post('http://localhost:5000/chartProCC', {email,ora});
    ...
    return(
        <div className='flex'>
            <Sidebar sidebar={1} link={12}/> {/*componente sidebar con relativi props*/}
