...
function Garage() {
    ...
    useEffect(()=>{
        ...
        const garage = async () => {
            ...
            const response = await axios.post('http://localhost:5000/garageSU', {email});
            // Se la risposta del server contiene dati validi, creo le tabelle per visualizzarli.
            if(response.data==="nessun garage"){
                document.getElementById("table").style.display="none";
                document.getElementById("noGA").style.display="block";
            }else{
                for(var i=0;i<response.data.length;i++){
                    createTable("tableGarage",response.data[i]);
                }
            }
          }
    ...
    const eliminaGa = async (idGa,row) => {
        try {
            //richiesta http per prendere i dati del profilo e azienda
            const response = await axios.delete('http://localhost:5000/deleteGa', {data:{id:idGa}});
            // Se la risposta del server contiene dati validi, creo le tabelle per visualizzarli.
    ...
    return(
        <div className='flex'>
            <Sidebar sidebar={0} link={9}/>  {/*componente sidebar con relativi props*/}
            <div className="p-7 text-2xl font-semibold flex-1 h-full overflow-x-auto">
                ...
                <div className="alert alert-danger text-center" role="alert" style={{ display: "none",marginTop:"30px"}} id="noGA" >
