...
function ProfiloCF() {
    ...
    useEffect(()=>{
        ...
        const profilo = async () => {
            ...
            const response = await axios.post('http://localhost:5000/profiloCF', {email});
        ...
        const famigliaCF = async () => {
            //Prendo l'email dell'utente loggato dallo stato di autenticazione
            const email = auth.email;
            //richiesta http per prendere i dati del profilo e azienda
            const response = await axios.post('http://localhost:5000/famigliaCF', { email });
            // Se la risposta del server contiene dati validi, creo le tabelle per visualizzarli.
            if (response.data) {
        ...
        famigliaCF(); //richiamo la funzione famigliaCF
    },[auth])
    ...
    return(
        <div className='flex'>
            ...
            <div className="p-7 text-2xl font-semibold flex-1 h-full overflow-x-auto">
                ...
                <header className="bg-grey text-center text-blue-600" style={{marginTop:"50px"}}>
                    Famiglia
                </header>
                <div className="relative overflow-x-auto shadow-md sm:rounded-lg" style={{paddingTop:"30px"}}>
